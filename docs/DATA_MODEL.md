...
- `legal_draft_recipients` stores append-only, owner/case-scoped recipient
  revisions. `legal_draft_snapshots` stores exact generated bytes, hashes,
  recipient and analysis/input revisions, provenance, and review state. Both
  tables participate in owner export plus case/account erasure.
...
- Legal drafts are unique by owner, case, type, and generation revision. A
  second uniqueness boundary gives each type a monotonic version, while a
