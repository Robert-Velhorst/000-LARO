...
Scanned **169** runtime files (server, src-main, shared) and **271** other files (src, scripts, tests).
...
| `server/outreachSend.ts` | 277 | `* `sender` is injectable (tests pass a fake); production uses systemEmail.` |
...
## Markers in tests / scripts / renderer (272) — reported, not failing
...
| File | Marker count |
|---|---|
| `tests/backend/trelloIngress.test.ts` | 43 |
| `tests/backend/googleDriveAccountSelection.test.ts` | 21 |
