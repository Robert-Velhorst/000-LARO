...
| D14 | Persisted NL/EN runtime exists, but legacy renderer surfaces still contain uncatalogued user-facing copy | Medium | In progress: settings, auto-collection, the mounted case workspace, and document reconstruction are catalog-driven and browser-tested; the stabilization gate blocks drift against an explicit 1,591-candidate/56-file baseline until staged migration reaches zero or values are reviewed as non-translatable |
