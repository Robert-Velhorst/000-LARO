...
| D14 | Persisted NL/EN runtime exists, but legacy renderer surfaces still contain uncatalogued user-facing copy | Medium | In progress: settings, auto-collection, and the mounted case workspace are catalog-driven and browser-tested; the stabilization gate blocks drift against an explicit 1,693-candidate/57-file baseline until staged migration reaches zero or values are reviewed as non-translatable |
