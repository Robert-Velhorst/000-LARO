...
| D14 | Persisted NL/EN runtime exists, but legacy renderer surfaces still contain uncatalogued user-facing copy | Medium | In progress: settings, auto-collection, the mounted case workspace, document reconstruction, document-source jobs, evidence coverage, and case-scoped public research are catalog-driven and browser-tested; the stabilization gate blocks drift against an explicit 1,318-candidate/52-file baseline until staged migration reaches zero or values are reviewed as non-translatable |
